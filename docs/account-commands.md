...
Re-check already stored pending evidence without looking up UUIDs:
...
`verify-attestation` exits `0` only when the resulting status is `valid`. It
exits `22` for `pending`, `untrusted`, `invalid`, `expired`, or `error`, so CI
does not treat stored but unverified provenance as trusted.

The accepted key type is ECDSA P-256, which is the key type generated by
`cosign generate-key-pair`. Uploads whose bundle key hint matches this active
organisation key are verified cryptographically.

## `upload-attestation`

Upload a Cosign/Sigstore bundle or DSSE in-toto JSON file for an existing
product version.
...
craevidence upload-attestation \
  ...
  --file <bundle-or-dsse.json>
```

Cosign v3 writes the Sigstore Bundle format accepted by this command by
default. With Cosign 2.6.x, add `--new-bundle-format=true`. Sign the release
artifact itself so the subject digest identifies the built output:
...
export CRA_EVIDENCE_PRODUCT=product-slug
export CRA_EVIDENCE_VERSION=1.0.0

craevidence upload-attestation \
  --file provenance.sigstore.json
```

An accepted upload can remain `pending`. This means the provenance is stored,
not that its signer has been verified. A matching active public key registered
with `trust-attestation-key` changes a correct key-based bundle to `valid`.
