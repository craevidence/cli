# Vendored CSAF 2.0 JSON schema

`csaf_json_schema.json` is the normative JSON schema for the Common Security
Advisory Framework (CSAF) Version 2.0, an OASIS Standard. It is reproduced here
unmodified and is used only by the test suite to validate the CSAF documents
this CLI emits (`draft advisory` and `draft vex --format csaf`). It lives under
`tests/` and is not part of the published package distribution.

- Schema `$id`: https://docs.oasis-open.org/csaf/csaf/v2.0/csaf_json_schema.json
- Standard: https://docs.oasis-open.org/csaf/csaf/v2.0/os/csaf-v2.0-os.html

The complete OASIS notice below is reproduced as required by its own terms.
